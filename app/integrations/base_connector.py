...
class SyncResult:
    ...
    synced_at: datetime = field(default_factory=utcnow_naive)
...
class BaseConnector(ABC):
    ...
    def _do_sync(self, since: Optional[datetime] = None) -> SyncResult:
        ...
        return SyncResult(
            success=False, errors=["Sync not supported by this connector."]
        )
    ...
    def connect(self, credentials: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            if result:
                ...
                self._connected_at = utcnow_naive()
    ...
    def sync(self, since: Optional[datetime] = None) -> SyncResult:
        ...
        if not self._circuit.allow_request():
            return SyncResult(
                success=False, errors=["Circuit breaker is open — too many failures."]
            )
        try:
