...
def create_schema(project_id):
    project = _get_project_or_404(project_id)
    payload, status_code = extraction_service.create_schema(project, request.get_json() or {})
    return jsonify(payload), status_code
...
def list_extractions(project_id):
    project = _get_project_or_404(project_id)
    payload, status_code = extraction_service.list_extractions(project, request.args.get('schema_id'))
    return jsonify(payload), status_code


@extraction_bp.route('/<int:project_id>/extractions/<int:result_id>/validate', methods=['POST'])
...
@extraction_bp.route('/<int:project_id>/schemas/<int:schema_id>/fields', methods=['GET'])
...
@extraction_bp.route('/<int:project_id>/schemas/<int:schema_id>/fields', methods=['POST'])
def create_schema_field(project_id, schema_id):
    ...
    """
    project = _get_project_or_404(project_id)
    payload, status_code = extraction_service.create_schema_field(project, schema_id, request.get_json() or {})
    return jsonify(payload), status_code


@extraction_bp.route('/<int:project_id>/extractions/<int:result_id>/field-values', methods=['GET'])
def get_extracted_field_values(project_id, result_id):
    """Get all field values from an extraction result."""
    project = _get_project_or_404(project_id)
    payload, status_code = extraction_service.get_extracted_field_values(project, result_id)
    return jsonify(payload), status_code


@extraction_bp.route('/<int:project_id>/schemas/<int:schema_id>/validators', methods=['GET'])
