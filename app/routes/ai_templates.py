...
def execute_template():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"error": "Failed to create execution record"}), 500

    return jsonify({"execution_id": execution.id, "status": "pending"})
    ...
    def generate():
        try:
            ...
            try:
                ...
                db.session.rollback()
                yield f"event: error\ndata: {json.dumps({'error': 'Failed to update execution status'})}\n\n"
                return
            ...
            for chunk in generate_with_llm(
                ...
                max_tokens=execution.input_data.get("_maxLength", 500)
                * 2,  # Rough conversion
                stream=True,
            ):
            ...
            try:
                ...
                db.session.rollback()
                yield f"event: error\ndata: {json.dumps({'error': 'Failed to save execution result'})}\n\n"
                return

            # Send completion
            completion_data = json.dumps(
                {"success": True, "tokens": token_count, "model": "GPT-4"}
            )
        ...
        [
            {
                ...
                "document_count": WorkbookDocument.query.filter_by(
                    workbook_id=wb.id
                ).count(),
...
def save_to_workbook():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"error": "Failed to save document to workbook"}), 500

    return jsonify(
        {"success": True, "document_id": document.id, "workbook_id": workbook_id}
    )
