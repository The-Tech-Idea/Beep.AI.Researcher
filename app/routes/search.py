...
def _local_search(project, q):
    ...
    for d in docs:
        ...
        results.append({'document_id': d.id, 'filename': d.filename, 'snippet': snippet, 'offset': idx})
...
def _rag_search(project, q):
    ...
    for r in results:
        if not isinstance(r, dict):
            continue
        entries.append({
            'document_id': r.get('document_id') or r.get('metadata', {}).get('researcher_doc_id'),
...
def run_search(project_id):
    ...
    source_hint = (payload.get('source') or query_args.get('source') or '').strip().lower()
    if not q:
    ...
    if source_hint == 'rag':
        return jsonify({'results': local_results, 'source': 'local', 'note': 'Document library search is unavailable, so these are local file matches.'})
