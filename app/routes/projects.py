...
def list_rag_collections_for_project():
    ...
    if not is_configured():
        return jsonify({'collections': [], 'message': 'Connect the document library service in Admin settings first.'})
...
def create_project():
    ...
    payload['start_url'] = url_for('project_start.project_start_page', project_id=project.id)
    payload['overview_url'] = url_for('researcher.project_overview', project_id=project.id)
    payload['settings_url'] = url_for('researcher.project_settings', project_id=project.id)
...
def get_project_rag_organization_profile(project_id):
    ...
    if not project.collection_id:
        return jsonify({'success': False, 'error': 'Project is not linked to a document library yet.'}), 400
    if not is_configured():
        return jsonify({'success': False, 'error': 'The document library service is not configured.'}), 503
    ...
    return jsonify({
        ...
        'graph_reading_mode': resolve_graph_reading_mode(
            collection_graph_extraction_profile_id=profile.get('collection_graph_extraction_profile_id'),
            database_default_graph_extraction_profile_id=profile.get('database_default_graph_extraction_profile_id'),
            effective_graph_extraction_profile_id=profile.get('graph_extraction_profile_id'),
            available_profiles=available_profiles,
        ),
    })
...
def update_project_rag_organization_profile(project_id):
    ...
    data = request.get_json() or {}
    organization_profile = data.get('organization_profile') if isinstance(data.get('organization_profile'), dict) else {}
    organization_profile = dict(organization_profile)
    metadata_schema = data.get('metadata_schema') if isinstance(data.get('metadata_schema'), dict) else None
    ...
    if 'graph_extraction_profile_id' in data:
        graph_extraction_profile_id = (data.get('graph_extraction_profile_id') or '').strip()
    if metadata_schema is None and isinstance(organization_profile.get('metadata_schema'), dict):
    ...
    return jsonify({
        ...
        'graph_reading_mode': resolve_graph_reading_mode(
            collection_graph_extraction_profile_id=(
                result.get('organization_profile', {}).get('collection_graph_extraction_profile_id')
                if isinstance(result, dict) else None
            ),
            database_default_graph_extraction_profile_id=(
                result.get('organization_profile', {}).get('database_default_graph_extraction_profile_id')
                if isinstance(result, dict) else None
            ),
            effective_graph_extraction_profile_id=(
                result.get('organization_profile', {}).get('graph_extraction_profile_id')
                if isinstance(result, dict) else None
            ),
            available_profiles=available_profiles,
        ),
...
@projects_bp.route('/<int:project_id>/rag/documents/<document_id>/chunks', methods=['GET'])
@login_required
def get_project_rag_document_chunks(project_id, document_id):
    ...
    ok, result = get_collection_document_chunks(
        ...
        document_id,
        user_id=str(current_user.id),
        include_content=(request.args.get('include_content') or '').strip().lower() in {'1', 'true', 'yes', 'on'},
        preview_chars=request.args.get('preview_chars', type=int),
    )
    if not ok:
        return jsonify({'success': False, 'error': str(result)}), 502
    return jsonify(result if isinstance(result, dict) else {'success': True, 'result': result})


@projects_bp.route('/<int:project_id>/rag/documents/<document_id>/lineage', methods=['GET'])
@login_required
...
def update_project(project_id):
    ...
    if 'chunk_template_slug' in data:
        project.chunk_template_slug = (data['chunk_template_slug'] or '').strip() or None
...
def list_available_chunk_templates():
    ...
    """
    if not is_configured():
        return jsonify({'templates': [], 'message': 'Document library service not configured.'})

    ok, templates = list_chunk_templates()
    if not ok:
        return jsonify({'templates': [], 'error': str(templates)})

    include_researcher = request.args.get('include_researcher', 'true').lower() not in {'0', 'false', 'no'}
...
def provision_researcher_templates():
    ...
    """
    if not is_configured():
        return jsonify({'success': False, 'error': 'Document library service not configured.'}), 503
