...
def _export_json(project):
    out = {
        ...
        "codes": [
            {"id": c.id, "name": c.name, "color": c.color} for c in project.codes
        ],
    }
...
def _export_sav(project):
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"{project.name}_export.sav",
    )
...
def _export_bundle(project):
    ...
    audit_logs = (
        AuditLog.query.filter_by(project_id=project.id)
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as arch:
        ...
        _write_json(
            arch, "project/references.json", [reference_dict(r) for r in references]
        )
        _write_json(
            arch,
