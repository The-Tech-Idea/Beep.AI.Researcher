...
def regression(project_id):
    ...
    if not x_cols:
        return jsonify(
            {"error": "x_columns required (list of predictor column names)"}
        ), 400
    ...
    if n < len(x_cols) + 2:
        return jsonify(
            {"error": f"Not enough complete rows for regression (found {n})"}
        ), 400
        ...
        return jsonify(
            {
                ...
                "coefficients": {
                    x_cols[i]: round(float(coefs[i + 1]), 6) for i in range(len(x_cols))
                },
                "r_squared": round(r_squared, 6),
                "se_estimate": round(se, 6),
                "y_column": y_col,
