...
from app.models.researcher import (
    Code, ResearchProject, ResearchTask, ResearcherDocument,
    CodedReference, ExtractionResult, ExtractionSchema, Flashcard, Quiz,
    ResearchReportDraft, ChatSession,
)
...
def _is_partial():
    """True when SPA requests just the content fragment."""
    return (request.args.get('partial') or '').strip().lower() in ('1', 'true') \
...
def _project_sidebar_counts(project_id):
    ...
    return {
        'document_count': ResearcherDocument.query.filter_by(project_id=project_id).count(),
...
def index():
    ...
    if project_ids:
        document_counts = dict(db.session.query(ResearcherDocument.project_id, func.count(ResearcherDocument.id))
                               .filter(ResearcherDocument.project_id.in_(project_ids))
                               .group_by(ResearcherDocument.project_id)
                               .all())
        code_counts = dict(db.session.query(Code.project_id, func.count(Code.id))
                           .filter(Code.project_id.in_(project_ids))
                           .group_by(Code.project_id)
                           .all())
        task_counts = dict(db.session.query(ResearchTask.project_id, func.count(ResearchTask.id))
...
def project_overview(project_id):
    ...
    extraction_count = (db.session.query(func.count(ExtractionResult.id))
                        .join(ExtractionSchema, ExtractionResult.schema_id == ExtractionSchema.id)
                        .filter(ExtractionSchema.project_id == project.id).scalar() or 0)
    ...
    recent_documents = (ResearcherDocument.query
                        .filter_by(project_id=project.id)
                        .order_by(ResearcherDocument.created_at.desc())
                        .limit(3)
                        .all())
    recent_tasks = (ResearchTask.query
                    .filter_by(project_id=project.id)
    ...
    activities.sort(
        key=lambda item: (item.get('created_at').timestamp() if item.get('created_at') else 0),
        reverse=True,
    )
...
def project_documents(project_id):
    ...
    project = _get_project_or_404(project_id)
    documents = ResearcherDocument.query.filter_by(project_id=project.id).order_by(
        ResearcherDocument.created_at.desc()).all()

    return render_template('project/documents.html', project=project, documents=documents,
...
def project_search(project_id):
    """Search and AI chat page"""
    project = _get_project_or_404(project_id)
    chat_history = (ChatSession.query
                    .filter_by(project_id=project.id)
                    .order_by(ChatSession.created_at.desc())
                    .limit(10)
                    .all())
    return render_template('project/search.html', project=project, chat_history=chat_history,
                           active_page='search',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template())
...
def project_codes(project_id):
    ...
    return render_template('project/codes.html', project=project, codes=codes,
                           active_page='codes',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template())
...
def project_tasks(project_id):
    ...
    tasks_todo = ResearchTask.query.filter_by(project_id=project.id, status='todo').all()
    tasks_in_progress = ResearchTask.query.filter_by(project_id=project.id, status='in_progress').all()
    tasks_done = ResearchTask.query.filter_by(project_id=project.id, status='done').all()
...
def project_report(project_id):
    ...
    return render_template('project/report.html', project=project, report=report,
                           active_page='report',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template())


@dashboard_bp.route('/api/projects/<int:project_id>/report/draft', methods=['GET', 'PUT'])
@login_required
...
def write_section_api(project_id):
    ...
    if beep_ai_client.is_configured():
        ...
        if project.custom_instructions:
            context_bits.append(f'Project writing instructions: {project.custom_instructions}')
        ...
        if ok:
            return jsonify({
                ...
                'supporting_sources': merge_supporting_sources([], grounded_context),
            })
...
def project_settings(project_id):
    ...
    total_size = db.session.query(func.sum(ResearcherDocument.file_size)).filter_by(
...
def api_agent_plans_list():
    ...
    return jsonify(result if isinstance(result, dict) else {'success': True, 'sessions': result})
...
def api_agent_plans_create():
    ...
    if not ok:
        return jsonify({'success': False, 'error': result}), 502
    return jsonify(result if isinstance(result, dict) else {'success': True, 'data': result})
...
def document_map(project_id):
    project = _get_project_or_404(project_id)
    return render_template('project/document_map.html',
                           project=project,
                           active_page='map',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template(),
...
@dashboard_bp.route('/projects/<int:project_id>/codes')
@login_required
def code_browser(project_id):
    from app.models.researcher import Code, CodedReference, ResearcherDocument
    project = _get_project_or_404(project_id)
    codes = Code.query.filter_by(project_id=project.id, parent_id=None).all()
    code_refs = {}
    for c in codes:
        refs = CodedReference.query.filter_by(code_id=c.id).all()
        code_refs[c.id] = [_ref_meta(r) for r in refs]
        for ch in c.children:
            refs_ch = CodedReference.query.filter_by(code_id=ch.id).all()
            code_refs[ch.id] = [_ref_meta(r) for r in refs_ch]
    return render_template('code_browser.html', project=project, codes=codes, code_refs=code_refs,
                           active_page='codes',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template())
...
def _ref_meta(r):
    ...
    return {
        'id': r.id, 'document_id': r.document_id, 'start_offset': r.start_offset, 'end_offset': r.end_offset,
        'filename': doc.filename if doc else '?', 'snippet': snippet,
    }
...
def search_page(project_id):
    """Legacy route — redirect to modular search page."""
    from flask import redirect, url_for
    _get_project_or_404(project_id)
    return redirect(url_for('researcher.project_search', project_id=project_id))
...
def chat_page(project_id):
    from flask import redirect, url_for
    _get_project_or_404(project_id)
    return redirect(url_for('researcher.project_overview', project_id=project_id, openChat=1))
...
def data_page(project_id):
    ...
    return render_template('project/data.html', project=project, sources=sources, charts=charts,
                           active_page='data',
...
def extraction_page(project_id):
    ...
    return render_template('project/extraction.html',
                           project=project,
                           schemas=schemas,
...
def report_share_page(project_id):
    ...
    project = _get_project_or_404(project_id)
    return render_template('project/report_share.html',
                           project=project,
                           active_page='report',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template())
...
def email_report_api(project_id):
    ...
    if not is_configured():
        return jsonify({'success': False, 'error': 'Server email (SMTP) is not configured.'}), 500
        
    if not to_email or not content:
        return jsonify({'success': False, 'error': 'Email address and report content are required.'}), 400
...
def export_pdf_api(project_id):
    ...
    try:
        ...
        return jsonify({'error': 'PDF generation is currently unavailable. Install xhtml2pdf.'}), 500
...
def members_page(project_id):
    ...
    users = User.query.filter(User.is_active, User.id != project.owner_id).order_by(User.username).all()
...
def matrix_page(project_id):
    project = _get_project_or_404(project_id)
    return render_template('project/matrix.html',
                           project=project,
                           active_page='matrix',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template(),
...
def flashcards_page(project_id):
    project = _get_project_or_404(project_id)
    return render_template('project/flashcards.html',
                           project=project,
...
def quizzes_page(project_id):
    project = _get_project_or_404(project_id)
    return render_template('project/quizzes.html',
                           project=project,
                           active_page='quizzes',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template(),
...
def take_quiz_page(project_id, quiz_id):
    ...
    return render_template('project/take_quiz.html',
                           project=project,
                           quiz=quiz,
                           active_page='quizzes',
                           **_project_sidebar_counts(project.id),
...
def scheduled_reports_page(project_id):
    ...
    return render_template('project/scheduled_reports.html', project=project, reports=reports,
                           active_page='reports',
                           **_project_sidebar_counts(project.id),
                           base_template=_base_template())
...
def retention_page(project_id):
    project = _get_project_or_404(project_id)
    return render_template('project/retention.html', project=project,
...
def contradictions_page(project_id):
    project = _get_project_or_404(project_id)
    return render_template('project/contradictions.html', project=project,
...
def hallucination_audit_page(project_id):
    ...
    flagged = HallucinationAuditLog.query.filter_by(project_id=project.id, flagged=True).count()
    avg_score = db.session.query(func.avg(HallucinationAuditLog.grounding_score)).filter(
        HallucinationAuditLog.project_id == project.id
    ).scalar()
    recent = (HallucinationAuditLog.query
              .filter_by(project_id=project.id)
              .order_by(HallucinationAuditLog.created_at.desc())
              .limit(50).all())
    return render_template('project/hallucination_audit.html',
                           project=project,
                           total=total,
                           flagged=flagged,
...
def suggest_tasks_api(project_id):
    ...
    try:
        ...
        docs = ResearcherDocument.query.filter_by(project_id=project.id, status='ready').limit(5).all()
...
def project_references_page(project_id):
    ...
    from app.models.researcher import ResearcherDocument
    from app.services.reference_bibliography_service import get_bibliography_style_options
    project = _get_project_or_404(project_id)
    ...
    documents = ResearcherDocument.query.filter_by(project_id=project.id).order_by(ResearcherDocument.filename).all()
    return render_template(
