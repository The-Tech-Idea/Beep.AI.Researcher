...
def web_search(project_id):
    ...
    try:
        ...
        _publish_event('search.completed', {
            'project_id': project.id,
            'user_id': current_user.id,
            'query': query,
            'result_count': total,
...
@extended_search_bp.route('/projects/<int:project_id>/web-search/autocomplete', methods=['GET'])
@login_required
def search_autocomplete(project_id):
    ...
    source_ids = db.session.query(LibrarySource.id).filter_by(project_id=project.id).subquery()
    log_query = (
    ...
    if partial_q:
        log_query = log_query.filter(
            SourceImportLog.query.ilike(f'{partial_q}%')
        )
    ...
    suggestions = [q for q in unique_recent if partial_q.lower() in q.lower()] if partial_q else []
...
@extended_search_bp.route('/projects/<int:project_id>/web-search/sources', methods=['GET'])
@login_required
def list_searchable_sources(project_id):
    ...
    sources = LibrarySource.query.filter_by(
        project_id=project.id,
        is_active=True
    ).all()
...
@extended_search_bp.route('/projects/<int:project_id>/web-search/available', methods=['GET'])
@login_required
...
@extended_search_bp.route('/projects/<int:project_id>/web-search/popular', methods=['GET'])
@login_required
...
@extended_search_bp.route('/projects/<int:project_id>/web-search/filters/publication-types', methods=['GET'])
@login_required
...
@extended_search_bp.route('/projects/<int:project_id>/web-search/filters/languages', methods=['GET'])
@login_required
...
def add_search_result_to_library(project_id):
    ...
    ref_data = {
        ...
        'source_type': _normalize_book_source_type(data.get('source_type') or data.get('result_type')),
        'url': data.get('url') or '',
        'doi': data.get('doi') or '',
