...
def integration_update(service_id):
    ...
    data = {
        ...
        "oauth2_client_secret": request.form.get("oauth2_client_secret", "").strip()
        or None,
        ...
        "oauth2_redirect_uri": request.form.get("oauth2_redirect_uri", "").strip()
        or None,
        "global_api_key": request.form.get("global_api_key", "").strip() or None,
        "global_extra_config": request.form.get("global_extra_config", "").strip()
        or None,
    }
    try:
...
def integration_disable(service_id):
    ...
    try:
        ...
        db.session.rollback()
        flash(f"Error disabling {svc.name}.", "danger")
        return redirect(url_for("admin_integrations.integrations_list"))
...
def integration_users(service_id):
    ...
    svc = get_entity_or_404(GlobalIntegrationService, service_id)
    creds = (
        UserIntegrationCredential.query.filter_by(service_id=service_id, is_active=True)
...
@admin_integrations_bp.route(
    "/<int:service_id>/users/<int:user_id>/disconnect", methods=["POST"]
)
@login_required
@admin_required
def admin_disconnect_user(service_id, user_id):
    ...
    return redirect(
        url_for("admin_integrations.integration_users", service_id=service_id)
    )
