...
from app.models.researcher import (
    ResearchProject, Code, CodedReference, ResearcherDocument
)
...
def list_codes(project_id):
    ...
    for c in codes:
        ...
        out.append({
            **code_to_dict(c), 'reference_count': ref_count,
...
def apply_code(project_id):
    ...
    db.session.add(ref)
    db.session.commit()
    return jsonify({'id': ref.id, 'code_id': ref.code_id, 'document_id': ref.document_id}), 201


@codes_bp.route('/<int:project_id>/code/<int:ref_id>', methods=['DELETE'])
def remove_code(project_id, ref_id):
    project = _get_project_or_404(project_id)
    ref = CodedReference.query.join(Code).filter(
        Code.project_id == project.id, CodedReference.id == ref_id
    ).first_or_404()
...
def list_references(project_id, code_id):
    ...
    for r in refs:
        ...
        out.append({
            'id': r.id, 'document_id': r.document_id, 'filename': doc.filename if doc else None,
            'chunk_id': r.chunk_id, 'start_offset': r.start_offset, 'end_offset': r.end_offset,
            'snippet': snippet, 'memo': r.memo,
        })
