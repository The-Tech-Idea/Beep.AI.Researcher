        ...
        if ok:
            ...
            for item in items:
                item_doc_id = (
                    ...
                    or (item.get('metadata') or {}).get('researcher_doc_id')
                )
                # Skip self
                try:
            ...
            logger.warning('RAG related docs failed: %s — falling back to local', result)

    # ── Local fallback ─────────────────────────────────────────────────────
    others = ResearcherDocument.query.filter_by(project_id=project.id).filter(
        ResearcherDocument.id != doc.id
    ).all()
...
def find_citations(project_id):
    ...
    if not text:
        return jsonify({'error': 'Enter the passage you want supporting files for.'}), 400
