...
def remove_member(project_id, member_id):
    ...
    m = ProjectMember.query.filter_by(
        project_id=project.id, id=member_id
    ).first_or_404()
    db.session.delete(m)
    try:
        db.session.commit()
...
def add_comment(project_id):
    ...
    if parent_id:
        parent = ProjectComment.query.filter_by(
            id=parent_id, project_id=project.id
        ).first_or_404()
...
def get_comment(project_id, comment_id):
    project = get_project_or_404(project_id)
    c = ProjectComment.query.filter_by(
        id=comment_id, project_id=project.id
    ).first_or_404()
    d = _comment_to_dict(c)
    replies = (
        ProjectComment.query.filter_by(parent_id=c.id)
...
@collab_bp.route(
    "/<int:project_id>/comments/<int:comment_id>/resolve", methods=["POST"]
)
@login_required
