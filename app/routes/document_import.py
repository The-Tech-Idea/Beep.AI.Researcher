...
@document_import_bp.route(
    "/<int:project_id>/web-search/<string:result_id>/import", methods=["POST"]
)
@login_required
def import_search_result(project_id, result_id):
    ...
    filename = (
        payload.get("filename") or f"import_{result_id}.pdf"
        if pdf_url
        else payload.get("title", f"import_{result_id}")
    )
    ...
    doc = ResearcherDocument(
        ...
        or ("application/pdf" if pdf_url else "text/plain"),
        file_size=0,
    ...
    if pdf_url:
        ...
        try:
            ...
            job_id = queue_job(
                CoreJobType.PDF_DOWNLOAD.value, job_payload, priority=JobPriority.NORMAL
            )
        except Exception:
            # Fallback to generic extract document job if PDF job type unavailable
            job_id = queue_job(
                JobType.EXTRACT_DOCUMENT.value, job_payload, priority=JobPriority.NORMAL
            )
    else:
...
def batch_import(project_id):
    ...
    job_id = queue_job(
        "web_search_batch_import", job_payload, priority=JobPriority.NORMAL
    )
...
def import_stats(project_id):
    ...
    doc_counts = (
        ...
        .group_by(ResearcherDocument.source_type)
        .all()
    )
    ...
    log_stats = (
        ...
        .group_by(SourceImportLog.status)
        .all()
    )
    ...
    top_sources = (
        ...
        .filter_by(project_id=project_id)
        .order_by(LibrarySource.import_count.desc())
        .limit(5)
        .all()
    )
