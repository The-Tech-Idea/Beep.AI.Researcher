...
def user_create():
    ...
    if request.method == 'POST':
        ...
        from app.services.password_policy_service import validate_password, record_password_change
...
def user_mfa_reset(user_id):
    ...
    db.session.commit()
    flash(f'MFA cleared for "{user.username}". They can re-enroll on next login.', 'success')
    return redirect(url_for('admin.user_detail', user_id=user_id))
...
def user_impersonate(user_id):
    ...
    login_user(target)
    flash(f'Now acting as "{target.username}". Return to admin to stop impersonation.', 'warning')
...
def user_delete(user_id):
    ...
    if reassign_to_id:
        try:
            ...
            ResearchProject.query.filter_by(user_id=user_id).update({'user_id': reassign_to_id})
        except Exception:
            pass
...
def user_import():
    ...
    for i, row in enumerate(reader, start=2):
        ...
        role = Role.query.filter_by(name=role_name).first() if role_name else default_role
        pw_hash = generate_password_hash(password)
        user = User(
            username=username,
            email=email,
...
def impersonate_stop():
    ...
    impersonator_id = flask_session.pop('impersonating_as', None)
