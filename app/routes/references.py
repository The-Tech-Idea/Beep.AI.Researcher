...
from app.models.researcher import Code, Reference, ResearchProject, ResearchTask, ResearcherDocument
...
from app.services.citation_library_service import build_project_citation_library, set_reference_tags
...
def _get_project(project_id):
    return get_entity(ResearchProject, project_id)
...
def references_page():
    ...
    base_template = 'base_embed.html' if (
...
def list_references(project_id):
    ...
    return jsonify({
        'references': [reference_to_dict(ref) for ref in library_view['references']],
...
@references_bp.route('/researcher/projects/<int:project_id>/references/<int:reference_id>')
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/<int:reference_id>/external-attachments', methods=['GET'])
@login_required
def reference_external_attachments(project_id, reference_id):
    project = _get_project_or_404(project_id)
    reference = Reference.query.filter_by(project_id=project.id, id=reference_id).first_or_404()
    try:
...
@references_bp.route('/projects/<int:project_id>/references/zotero/status', methods=['GET'])
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/zotero/sync', methods=['POST'])
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/bibliography-preview', methods=['GET'])
@login_required
...
def import_project_references(project_id):
    ...
    payload = _get_payload()
    import_format = (payload.get('format') or request.args.get('format') or 'json').lower()
    content = payload.get('content')

    if not content and request.files.get('file'):
        upload = request.files['file']
        content = upload.read().decode('utf-8', errors='ignore')
        if import_format == 'json' and upload.filename and upload.filename.lower().endswith('.bib'):
            import_format = 'bibtex'
        elif import_format == 'json' and upload.filename and upload.filename.lower().endswith('.ris'):
            import_format = 'ris'
...
@references_bp.route('/projects/<int:project_id>/references/<int:reference_id>', methods=['PUT'])
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/<int:reference_id>', methods=['DELETE'])
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/<int:reference_id>/documents', methods=['GET'])
@login_required
def list_reference_documents(project_id, reference_id):
    ...
    links = [
        link.to_dict()
        for link in DocumentReference.query.filter_by(reference_id=reference.id).order_by(DocumentReference.id.desc()).all()
    ]
    return jsonify({'reference_id': reference.id, 'documents': links, 'citation_count': reference.citation_count}), 200


@references_bp.route('/projects/<int:project_id>/references/<int:reference_id>/link-document', methods=['POST'])
@login_required
def link_reference_document(project_id, reference_id):
    ...
    return jsonify({'ok': True, 'link': link.to_dict(), 'citation_count': reference.citation_count}), 201


@references_bp.route('/projects/<int:project_id>/references/<int:reference_id>/link-document/<int:document_id>', methods=['DELETE'])
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/validate-doi', methods=['POST'])
@login_required
...
@references_bp.route('/projects/<int:project_id>/references/validate-citations', methods=['POST'])
@login_required
