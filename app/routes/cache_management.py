...
def delete_cache_entry(project_id, cache_id):
    ...
    try:
        ...
        cache_entry = SearchCache.query.filter_by(
            id=cache_id, project_id=project_id
        ).first()
        if not cache_entry:
        ...
        return {"deleted": True, "message": f"Deleted cache entry {cache_id}"}, 200
...
def get_indexed_results(project_id):
    ...
    try:
        ...
        facet_values = {
            "providers": sorted(
                set(idx.provider for idx in all_indexes if idx.provider)
            ),
            "sources": sorted(set(idx.source for idx in all_indexes if idx.source)),
            "result_types": sorted(
                set(idx.result_type for idx in all_indexes if idx.result_type)
            ),
            "access_types": sorted(
                set(idx.access_type for idx in all_indexes if idx.access_type)
            ),
            "years": sorted(
                set(
...
def cache_config(project_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            return {"message": "Cache configuration updated", "config": config}, 200
