...
def project_health(project_id):
    ...
    try:
        rows = (
            db.session.query(ResearcherDocument.status, func.count(ResearcherDocument.id))
...
def project_cache_stats(project_id):
    ...
    valid_entries = [e for e in entries if e.expires_at and e.expires_at > utcnow_naive()]
