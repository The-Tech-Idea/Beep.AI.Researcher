...
def create_task(project_id):
    ...
    db.session.add(task)
    db.session.commit()
    emit_task_notification(task, 'created', actor_id=current_user.id if current_user.is_authenticated else None)
...
def update_task(project_id, task_id):
    project = _get_project_or_404(project_id)
    task = ResearchTask.query.filter_by(project_id=project.id, id=task_id).first_or_404()
    data = request.get_json() or {}
...
def list_task_notifications(project_id):
    project = _get_project_or_404(project_id)
    notifications = TaskNotification.query.join(ResearchTask).filter(ResearchTask.project_id == project.id).order_by(
