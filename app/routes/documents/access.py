...
doc_access_bp = Blueprint('document_access', __name__, url_prefix='/documents/<doc_id>/access')
...
def unshare_document_with_user(doc_id):
    ...
    if access.owner_id != user_id:
        return jsonify({
            'error': 'Forbidden',
            'message': 'Only document owner can change sharing'
        }), 403
    ...
    return jsonify({
        'success': True,
        'message': f"Removed sharing with user '{target_user}'"
    }), 200
...
def make_document_private(doc_id):
    ...
    if access.owner_id != user_id:
        return jsonify({
            'error': 'Forbidden',
            'message': 'Only document owner can change access'
        }), 403
