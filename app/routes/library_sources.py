...
from app.models.researcher import ResearchProject, LibrarySource, SourceConnection, SourceImportLog
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources', methods=['POST'])
@login_required
def create_source(project_id):
    ...
    if source_type not in valid_types:
        return jsonify({'error': f'Invalid source_type. Must be: {", ".join(valid_types)}'}), 400
    ...
    if existing:
        return jsonify({'error': f'Source "{data.get("name")}" already exists in this project'}), 409
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/<int:source_id>', methods=['GET'])
@login_required
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/<int:source_id>', methods=['PUT'])
@login_required
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/<int:source_id>', methods=['DELETE'])
@login_required
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/<int:source_id>/test', methods=['POST'])
@login_required
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/<int:source_id>/connections', methods=['GET'])
@login_required
def get_source_connections(project_id, source_id):
    ...
    connections = SourceConnection.query.filter_by(source_id=source.id).order_by(
        SourceConnection.tested_at.desc()
    ).limit(limit).all()
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/<int:source_id>/imports', methods=['GET'])
@login_required
def get_source_imports(project_id, source_id):
    ...
    imports = SourceImportLog.query.filter_by(source_id=source.id).order_by(
        SourceImportLog.imported_at.desc()
    ).limit(limit).all()
...
@library_sources_bp.route('/projects/<int:project_id>/library-sources/health', methods=['GET'])
@login_required
def get_all_sources_health(project_id):
    ...
    for source in sources:
        last_conn = SourceConnection.query.filter_by(source_id=source.id).order_by(
            SourceConnection.tested_at.desc()
        ).first()
