...
def index():
    ...
    try:
        from app.models.researcher import ResearchDocument
        doc_count = ResearchDocument.query.count()
        total_storage = (
            db.session.query(sqlfunc.sum(ResearchDocument.file_size)).scalar() or 0
        )
    except Exception:
        doc_count = 0
        total_storage = 0
