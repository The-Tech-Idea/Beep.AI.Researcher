...
from app.models.researcher import (
    ResearchProject, ResearchBrief, Claim, ClaimEvidence, EvidenceItem, ReviewStep
)
...
def list_briefs(project_id):
    ...
    return jsonify({'briefs': [b.to_dict() for b in q.order_by(ResearchBrief.created_at.desc()).all()]})
...
def get_brief(project_id, brief_id):
    ...
    brief = ResearchBrief.query.filter_by(id=brief_id, project_id=project_id).first_or_404()
...
def update_brief(project_id, brief_id):
    ...
    ALLOWED = ('title', 'sector', 'summary_text', 'status',
               'compliance_frameworks', 'key_findings', 'llm_model_used')
...
def update_claim(project_id, claim_id):
    ...
    ALLOWED = ('claim_text', 'claim_type', 'sector', 'verdict', 'confidence_score', 'source_brief_id')
    for field in ALLOWED:
        if field in data:
...
@lifecycle_bp.route('/<int:project_id>/claims/<int:claim_id>/evidence', methods=['POST'])
def link_evidence_to_claim(project_id, claim_id):
    """Attach an EvidenceItem to a Claim with a role annotation."""
    _get_project_or_404(project_id)
    ...
    existing = ClaimEvidence.query.filter_by(claim_id=claim_id, evidence_id=evidence_id).first()
    if existing:
...
def update_evidence(project_id, ev_id):
    ...
    ALLOWED = ('claim_text', 'verbatim_quote', 'strength', 'direction',
               'evidence_type', 'source_location', 'extraction_method',
               'confidence_score', 'tags', 'document_id')
    for field in ALLOWED:
        if field in data:
...
def create_review(project_id):
    ...
    VALID_STAGES = ('identification', 'screening', 'eligibility', 'included', 'excluded')
...
def sign_off_review(project_id, step_id):
    """Formally sign off a review step, locking the decision.

    Body: { "decision": "pass|exclude|uncertain", "notes": "...", "performed_by": 1 }
    21 CFR Part 11 requirement: record IP address of signer.
    """
    _get_project_or_404(project_id)
    step = ReviewStep.query.filter_by(id=step_id, project_id=project_id).first_or_404()
    ...
    # 21 CFR Part 11 — capture signer IP
    if hasattr(step, 'signature_ip'):
        step.signature_ip = request.remote_addr

    # Mark signed-off if column exists (added in Phase E)
