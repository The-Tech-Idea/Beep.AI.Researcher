...
def _migrate_user_columns(db):
    ...
    cols = [
        (
            "email_verified",
            "BOOLEAN DEFAULT 1",
        ),  # 1 for existing users (backward compat)
...
def _migrate_feed_recommendation_columns(db):
    ...
    try:
        ...
        if "feed_recommendations" not in inspector.get_table_names():
            return
        existing_columns = {
            column["name"] for column in inspector.get_columns("feed_recommendations")
        }
    except Exception:
        db.session.rollback()
        return
    ...
    for column_name, column_type in columns:
        ...
        try:
            db.session.execute(
                text(
                    f"ALTER TABLE feed_recommendations ADD COLUMN {column_name} {column_type}"
                )
            )
            db.session.commit()
        except Exception:
...
def create_app(config_name=None):
    ...
    with app.app_context():
        ...
        from app.models.researcher import (
            ...
            LibrarySource,
            SourceConnection,
            SourceImportLog,  # Phase 2.2 Library sources
            SearchCache,
            SearchIndex,  # Phase 2.5 Search caching & indexing
        )
        ...
        if not app.config.get("TESTING", False):
            try:
                ...
                try:
                    ...
                    app.logger.warning(
                        "db.create_all() skipped (tables already exist): %s",
                        _create_all_err,
                    )
        ...
        if not app.config.get("TESTING", False):
            try:
                from app.services.cache_event_handlers import (
        ...
        try:
            from app.services.scheduled_report_service import (
                initialize_scheduled_report_runtime,
            )

            initialize_scheduled_report_runtime(
                app, start_dispatcher=not app.config.get("TESTING", False)
            )
        except Exception as e:
    ...
    from app.routes.cache_management import (
        cache_management_bp,
    ...
    from app.routes.research_interests import (
        research_interests_bp,
    )  # Phase 1 AI Discovery
    from app.routes.synthesis import synthesis_bp  # Phase 2 Evidence Synthesis
    from app.routes.knowledge_map import knowledge_map_bp  # Phase 3 Knowledge Map
    from app.routes.admin.admin_packages import admin_packages_bp  # Optional packages
    ...
    def check_setup():
        ...
        if request.endpoint and (
            "setup" in request.endpoint or request.endpoint == "static"
        ):
            return None
    ...
    def inject_locale():
        ...
        ui_features = {
            "ai_discovery_enabled": runtime_config.is_feature_enabled(
                "ai_discovery_enabled"
            ),
            "chat_enabled": runtime_config.is_feature_enabled("chat_enabled"),
            "web_search_enabled": runtime_config.is_feature_enabled(
                "web_search_enabled"
            ),
            "plugins_enabled": runtime_config.is_feature_enabled("plugins_enabled"),
            "notifications_enabled": runtime_config.is_feature_enabled(
    ...
    def set_locale(locale):
        ...
        next_url = (
            request.args.get("next") or request.referrer or url_for("researcher.index")
        )
    ...
    app.register_blueprint(
        cache_management_bp, url_prefix="/projects"
    )  # Phase 2.5 Cache management
    app.register_blueprint(
        video_sources_bp, url_prefix="/projects"
    )  # Phase 03 video ingest
    ...
    app.register_blueprint(synthesis_bp)  # Phase 2 Evidence Synthesis
    app.register_blueprint(knowledge_map_bp)  # Phase 3 Knowledge Map
    app.register_blueprint(admin_packages_bp)  # Optional packages
    ...
    # ── Error handlers ─────────────────────────────────────────────────────
    # Register standard HTTP error handlers (JSON for APIs, HTML for browsers)
    ...
    from app.services.container_setup import setup_container
