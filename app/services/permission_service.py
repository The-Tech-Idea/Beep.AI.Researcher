...
class PermissionService:
    ...
    def user_has_permission(
        ...
        if scope == 'global':
            ...
            if scope_id is not None:
                scoped_match = (UserRole.scope == scope) & (UserRole.scope_id == str(scope_id))
    ...
    @staticmethod
    def can_access_document(user_id: str, document_id: str, access_type: str = 'read') -> bool:
        ...
        if access_level == AccessLevel.GROUP.value:
            ...
            groups = shared_with.get('groups', []) if isinstance(shared_with, dict) else []
    ...
    def get_accessible_documents(user_id: str, project_id: str = None) -> list:
        ...
        query = DocumentAccess.query.filter(
            ...
            (DocumentAccess.access_level == AccessLevel.PUBLIC.value)  # Public documents
        )
    ...
    @staticmethod
    def get_user_roles(user_id: str, scope: str = 'global', scope_id: str = None) -> list:
        ...
        for role in roles:
            if role.permissions:
                if '*:*' in role.permissions:
                    ...
                    return {
                        'document:read', 'document:write', 'document:share', 'document:export',
