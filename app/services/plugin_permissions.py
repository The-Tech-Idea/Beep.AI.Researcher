...
class PluginPermissionService:
    ...
    def grant_permission(
        ...
        try:
            ...
            if existing:
                ...
                existing.updated_at = PluginPermissionService._utcnow()
                db.session.commit()
    ...
    @staticmethod
    def check_user_access(user_id: int, plugin_id: int, required_action: str = 'execute') -> Tuple[bool, str, AccessLevel]:
    ...
    def get_audit_logs(
        ...
        try:
            ...
            if days:
                cutoff = PluginPermissionService._utcnow() - timedelta(days=days)
            ...
            logs = query.order_by(PluginAudit.timestamp.desc()).limit(limit).offset(offset).all()
    ...
    def cleanup_expired_assignments() -> Tuple[bool, str, int]:
        ...
        try:
            expired = PluginRoleAssignment.query.filter(
                PluginRoleAssignment.expiry_date < PluginPermissionService._utcnow()
            ).all()
            ...
            return True, f"Cleaned up {deleted_count} expired assignments", deleted_count
