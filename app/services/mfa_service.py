...
def get_totp_provisioning_uri(user: "User", secret: str | None = None) -> str:
    """Build the otpauth:// URI for QR code display.

    Args:
        user: The user object.
        secret: Optional explicit secret to use. If not provided, reads from user.mfa_totp_secret.
    """
    issuer = (
        config_manager.get("mfa_totp_issuer")
        or config_manager.get("app_name")
        or "Beep.AI.Researcher"
    )
    secret = secret or user.mfa_totp_secret or ""
    try:
        import pyotp
...
def verify_totp_with_secret(secret: str, code: str) -> bool:
    """Verify a TOTP code against an explicit secret without mutating any user object.

    Used during setup to verify the code before committing the secret.
    """
    ...
    try:
        ...
        return _manual_totp_verify(secret, code)
...
def send_sms_otp(user: "User") -> tuple[bool, str]:
    ...
    if not phone:
        return (
            False,
            "No phone number on file. Please add a phone number to your profile.",
        )
...
def verify_backup_code(user: "User", code: str) -> bool:
    ...
    for i, h in enumerate(hashes):
        if check_password_hash(h, code):
            hashes.pop(i)
            user.mfa_backup_codes_hash = json.dumps(hashes)
            user.mfa_backup_codes_remaining = max(
                0, (user.mfa_backup_codes_remaining or 1) - 1
            )
            return True
