...
def _normalize_source_type(raw_type: str | None) -> str:
    value = (_clean_value(raw_type) or "other").lower()
    mapping = {
        "article": "journal",
...
def _ensure_unique_citation_key(project_id: int, preferred_key: str | None) -> str:
    base = _clean_value(preferred_key) or f"ref_{project_id}_{int(time.time())}"
    key = base
    i = 1
...
def create_reference(project: ResearchProject, data: dict, *, commit: bool = True) -> Reference:
    """Persist a reference record for a project."""
    source = _clean_value(data.get('source') or data.get('publication'))
    reference = Reference(
        project_id=project.id,
        document_id=int(data.get('document_id')) if data.get('document_id') else None,
        title=_clean_value(data.get('title')) or project.name,
        source=source,
        publication=source,
        ...
        doi=_clean_value(data.get('doi')),
        url=_clean_value(data.get('url')),
        abstract=_clean_value(data.get('abstract')),
        volume=_clean_value(data.get('volume')),
        issue=_clean_value(data.get('issue')),
        pages=_clean_value(data.get('pages')),
        citation=_clean_value(data.get('citation')),
        notes=_clean_value(data.get('notes')),
    )
...
def reference_to_dict(reference: Reference) -> dict:
    return {
        ...
        'document_filename': reference.document.filename if reference.document else None,
...
def link_reference_to_document(reference: Reference, document_id: int, payload: dict | None = None) -> DocumentReference:
    data = payload or {}
    link = DocumentReference.query.filter_by(reference_id=reference.id, document_id=document_id).first()
    if not link:
        link = DocumentReference(reference_id=reference.id, document_id=document_id)
        db.session.add(link)
    link.citation_context = _clean_value(data.get("citation_context"))
    ...
    if data.get("citation_type"):
        link.citation_type = _clean_value(data.get("citation_type")) or "direct"

    db.session.flush()
    reference.citation_count = DocumentReference.query.filter_by(reference_id=reference.id).count()
    reference.last_citation_date = _utcnow()
    db.session.commit()
    return link
...
def unlink_reference_from_document(reference: Reference, document_id: int) -> bool:
    ...
    db.session.flush()
    reference.citation_count = DocumentReference.query.filter_by(reference_id=reference.id).count()
    reference.last_citation_date = _utcnow() if reference.citation_count else None
    db.session.commit()
    return True
...
def validate_citation_batch(project: ResearchProject) -> dict:
    ...
    summary = {'total': len(references), 'valid': 0, 'invalid': 0, 'skipped': 0, 'details': []}
