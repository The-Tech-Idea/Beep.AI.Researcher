...
class MonitoringService:
    ...
    def analyze_trends(self, plugin_id, metric_type, days=7):
        ...
        """
        try:
            since_date = utcnow_naive() - timedelta(days=days)
    ...
    def create_performance_benchmark(self, plugin_id, plugin_name):
        ...
        try:
            ...
            recent_date = utcnow_naive() - timedelta(days=30)
            metrics = JobMetrics.query.filter(
                and_(
                    JobMetrics.plugin_id == plugin_id,
            ...
            benchmark = PerformanceBenchmark.query.filter_by(
                plugin_id=plugin_id
            ).first()
            ...
            if benchmark:
                ...
                benchmark.min_execution_time = (
                    exec_min if exec_min != float("inf") else None
                )
                benchmark.max_execution_time = (
                    exec_max if exec_max != float("-inf") else None
                )
                benchmark.avg_memory_used = mem_avg
                benchmark.max_memory_used = (
                    mem_max if mem_max != float("-inf") else None
                )
    ...
    def update_system_health(self):
        ...
        try:
            ...
            thread_count = (
                self.psutil.active_children().__len__()
                if hasattr(self.psutil, "active_children")
                else 0
            )
            ...
            all_metrics = JobMetrics.query.filter(
                JobMetrics.recorded_at >= (utcnow_naive() - timedelta(hours=1))
            ).all()
    ...
    def get_system_health(self, hours=1):
        ...
        try:
            ...
            latest = (
                SystemHealth.query.filter(SystemHealth.recorded_at >= since)
                .order_by(desc(SystemHealth.recorded_at))
                .first()
            )
    ...
    def check_performance_alerts(self):
        ...
        try:
            ...
            return (
                True,
                f"Alert check completed, {alerts_created} alerts created",
                alerts_created,
            )
        except Exception as e:
            db.session.rollback()
    ...
    def get_performance_alerts(
        self, status=None, alert_type=None, severity=None, limit=100
    ):
        """
        ...
        try:
            ...
            alerts = (
                query.order_by(desc(PerformanceAlert.created_at)).limit(limit).all()
            )
    ...
    def get_dashboard_metrics(self, timeframe="1h"):
        ...
        try:
            ...
            # Job statistics
            total_executed = BatchJob.query.filter(BatchJob.created_at >= since).count()
            ...
            return {
                ...
                "stats": {
                    ...
                    "failed_jobs": failed,
                    "success_rate": (completed / total_executed * 100)
                    if total_executed > 0
                    else 0,
    ...
    def get_plugin_performance_report(self, plugin_id, days=30):
        ...
        try:
            ...
            # Get recent metrics
            metrics = JobMetrics.query.filter(
                and_(JobMetrics.plugin_id == plugin_id, JobMetrics.recorded_at >= since)
            ).all()
            ...
            trends = self.analyze_trends(
                plugin_id, MetricType.EXECUTION_TIME.value, days
            )
            ...
            return {
                ...
                "report_date": utcnow_naive().isoformat(),
            }
        except Exception as e:
            return {"error": str(e)}
