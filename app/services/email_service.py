...
def send_email(
    subject: str, body: str, recipients: List[str], html_body: Optional[str] = None
...
def _build_mime(
    subject: str, body: str, html_body: Optional[str], recipients: List[str]
) -> MIMEMultipart:
...
def _send_smtp(
    ...
    port = int(
        config_manager.get_setting("smtp_port", default=587, env_var="SMTP_PORT") or 587
    )
    user = config_manager.get_setting("smtp_user", env_var="SMTP_USER") or ""
    password = (
        config_manager.get_setting("smtp_password", env_var="SMTP_PASSWORD") or ""
    )
    use_tls = config_manager.get_setting(
        "smtp_use_tls", default=True, env_var="SMTP_USE_TLS"
    )
...
def _send_ms365_graph(
    ...
    try:
        ...
        return (
            False,
            "oauth2_ms365 requires 'msal' and 'requests': pip install msal requests",
        )
