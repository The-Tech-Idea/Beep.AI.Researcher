...
class ReadingListService:
    ...
    def __init__(
        self,
        reading_list_repo=None,
        feed_repo=None,
        reference_repo=None,
        project_repo=None,
    ):
        self._reading_list_repo = reading_list_repo
        self._feed_repo = feed_repo
        self._reference_repo = reference_repo
        self._project_repo = project_repo

    # ── Repository accessors (lazy init for backward compat) ───────────────

    @property
    def _rl_repo(self):
        if self._reading_list_repo is None:
            from app.repositories.reading_list_repository import ReadingListRepository

            self._reading_list_repo = ReadingListRepository()
        return self._reading_list_repo
    ...
    def _proj_repo(self):
        if self._project_repo is None:
            ...
            self._project_repo = ProjectRepository(db.session)
    ...
    def list_items(
        ...
        *,
        ...
        items = self._rl_repo.get_by_user(user_id)
        if status:
            items = [item for item in items if item.status == status]

        items = sorted(items, key=lambda x: x.saved_at or x.created_at, reverse=True)
        if topic_tag:
        ...
        if source:
            ...
            items = [
                item
                for item in items
                if self.item_to_dict(item).get("source") == source_key
            ]
        return items
    ...
    def save_item(
        ...
        *,
        ...
        item = None
        if external_id:
            item = ReadingListItem.query.filter_by(
    ...
    def save_recommendation(
        ...
        self._fr_repo.commit()
        return item

    def save_recommendation_to_project(
        self, user_id: int, recommendation_id: int, project_id: int
    ) -> Reference:
        recommendation = self._get_recommendation(user_id, recommendation_id)
    ...
    def move_to_project(self, user_id: int, item_id: int, project_id: int) -> Reference:
        ...
        if item.reference_id:
            ...
            if (
                ...
                and existing_reference.project_id == project.id
            ):
    ...
    def _build_reference_payload(
        ...
        *,
        ...
        publication_date = parse_publication_date(
            getattr(recommendation, "publication_date", None)
        )
        payload = {
            "title": title,
            ...
            "source_type": source_to_reference_type(
                recommendation.source if recommendation else None
            ),
    ...
    def _get_or_create_reference(
        self, project: ResearchProject, payload: dict
    ) -> Reference:
        ...
        if reference is None:
            ...
            self._ref_repo.add(reference)
            self._ref_repo.flush()
        return reference

    def _latest_recommendation(
        self, user_id: int, external_id: str | None
    ) -> FeedRecommendation | None:
        if not external_id:
            return None
        return (
            FeedRecommendation.query.filter_by(user_id=user_id, external_id=external_id)
    ...
    def _find_existing_reference(
        ...
        if payload.get("doi"):
            match = self._ref_repo.get_by_doi(project_id, payload["doi"])
            if match:
                return match
        if payload.get("pubmed_id"):
            match = Reference.query.filter_by(
                project_id=project_id, pubmed_id=payload["pubmed_id"]
            ).first()
            if match:
                return match
        if payload.get("arxiv_id"):
            match = Reference.query.filter_by(
                project_id=project_id, arxiv_id=payload["arxiv_id"]
            ).first()
            if match:
                return match
        return Reference.query.filter_by(
            project_id=project_id, title=payload.get("title")
        ).first()

    def _topic_tags_from_recommendation(
        self, recommendation: FeedRecommendation
    ) -> list[str]:
