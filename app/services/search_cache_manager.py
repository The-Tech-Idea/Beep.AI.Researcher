...
class SearchCacheManager:
    ...
    def search_with_cache(
        ...
        # Try database cache
        if cache_enabled:
            cached_results = self._get_from_db_cache(
                project_id, query, filters, sources
            )
        ...
        results = self.search_manager.search(
            query, sources, filters, limit, deduplicate
        )
    ...
    def _get_from_db_cache(
        ...
        try:
            ...
            filters_part = (
                json.dumps(self._serialize_filters(filters), sort_keys=True)
                if filters
                else ""
            )
            # Hash must match SearchCache.__init__ logic: provider:query:filters_json
            query_hash = hashlib.sha256(
                f"{provider}:{query}:{filters_part}".encode()
            ).hexdigest()

            cache_entry = SearchCache.query.filter_by(
                project_id=project_id, provider=provider, query_hash=query_hash
            ).first()
    ...
    def _save_to_db_cache(
        ...
        try:
            filters_json = (
                json.dumps(self._serialize_filters(filters)) if filters else None
            )
    ...
    def _add_to_lru_cache(self, key: str, results: List[SearchResult]):
        ...
        if len(self.lru_cache) > self.max_lru_size:
            ...
            min_hits_key = min(
                self.lru_cache.keys(), key=lambda k: self.lru_cache[k]["hits"]
            )
    ...
    def invalidate_query_cache(
        self, project_id: int, query: str, filters: Optional[SearchFilter] = None
    ):
        ...
        try:
            ...
            if filters:
                filters_json = json.dumps(
                    self._serialize_filters(filters), sort_keys=True
                )
    ...
    def get_cache_stats(self, project_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            total_size_mb = sum(len(entry.results_json) for entry in cache_entries) / (
                1024 * 1024
            )
            ...
            return {
                ...
                "indexed_results": SearchIndex.query.filter_by(
                    project_id=project_id
                ).count(),
            }
    ...
    def get_faceted_search(self, project_id: int, **facets) -> List[SearchIndex]:
        ...
        try:
            ...
            query = query.order_by(
                SearchIndex.found_count.desc(), SearchIndex.citation_count.desc()
            )
