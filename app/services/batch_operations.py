...
class BatchOperationService:
    ...
    def create_batch_job(
        ...
        try:
            # Create job
            plugins_config = [
                {'plugin_id': pid, 'options': {}}
                for pid in plugins_list
            ]
    ...
    def start_batch_job(
        ...
        try:
            ...
            if BatchOperationService._status_value(job.status) != BatchJobStatus.pending.value:
    ...
    def pause_batch_job(job_id: int, user_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if BatchOperationService._status_value(job.status) != BatchJobStatus.running.value:
    ...
    def execute_batch_parallel(
        job_id: int,
        records: List[Dict],
        max_workers: int = MAX_PARALLEL_JOBS
    ...
    def _execute_plugin_on_record(
        ...
        """
        try:
            start_time = BatchOperationService._utcnow()
            ...
            execution_time = (BatchOperationService._utcnow() - start_time).total_seconds() * 1000
    ...
    def export_to_csv(job_id: int) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            output = io.StringIO()
            fieldnames = [
                'record_index', 'plugin_id', 'plugin_name',
    ...
    def get_batch_results(
        ...
        """
        try:
            query = BatchJobResult.query.filter(
                BatchJobResult.batch_job_id == job_id
            )
            ...
            results = query.order_by(BatchJobResult.created_at.desc()).limit(limit).offset(offset).all()
            
            return True, f"Retrieved {len(results)} of {total} results", [r.to_dict() for r in results]
    ...
    def get_batch_logs(
        ...
        try:
            ...
            logs = query.order_by(BatchJobLog.created_at.desc()).limit(limit).offset(offset).all()
            
            return True, f"Retrieved {len(logs)} of {total} logs", [log.to_dict() for log in logs]
    ...
    def cleanup_old_jobs(days: int = 30) -> Tuple[bool, str, int]:
        ...
        try:
            ...
            cutoff = BatchOperationService._utcnow() - timedelta(days=days)
            ...
            for job in old_jobs:
                ...
                BatchJobResult.query.filter(BatchJobResult.batch_job_id == job.id).delete()
